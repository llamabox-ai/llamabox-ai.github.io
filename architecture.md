...
LlamaBox is a **React Native Android application that runs large language models (LLMs) entirely on-device**, with no network calls for inference and no cloud dependency. You download a quantized GGUF model, the app loads it into RAM, and llama.cpp generates tokens directly on your phone's CPU.
...
### Why CPU-only by design?
Accelerated mobile compute is fragmented: drivers, library availability, and thermal behavior vary wildly between chipsets. To reach the widest range of devices — not just flagships with a working accelerated compute path — LlamaBox keeps inference strictly CPU-only. This is a scope decision, not a temporary limitation. It also removes an entire class of device-specific fallback and diagnostics work, keeping the app simpler and more predictable.

### Why is the vision encoder forced onto CPU?
Vision encoding is a one-shot burst. Running it on the CPU keeps the interface responsive during image processing and avoids device-specific graphics driver issues. The text decoder is the long-running part; the encoder runs once per image.
...
| `n_gpu_layers` | 0 | Not used; CPU-only by design |
...
**Limitations today**
- CPU-only inference by design (no GPU dependencies).
...
*Last updated: 2026-07-30. This document describes the architecture as of the closed beta; specifics may evolve before and after public source release.*